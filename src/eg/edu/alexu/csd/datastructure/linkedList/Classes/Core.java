...
public class Core {
    ...
    private static char getChar (boolean rIncluded) { //rIncluded means we accept r (result) as input
        Scanner sc = new Scanner(System.in);
    ...
    private static int[][] getNumbers (String s) {
        ...
        SLinkedList<Integer> ll = new SLinkedList<>(); //felt like this is better than looping using the scanner
        String input = s.replaceAll("[^\\d+.-]", " ");
        Scanner sc = new Scanner(input);
        try {
            while (sc.hasNext()) {
                ll.add(Math.round(sc.nextFloat()));
            }
        } catch (Exception e) {
        ...
        if (ll.size()%2 == 1) throw new RuntimeException("Invalid Input"); //odd number of entries
            ...
            int i = 0;
            while (ll.hasNext()) {
                numbers[0][i] = ll.getNext();
                numbers[1][i] = ll.getNext();
                i++;
            }
        }
